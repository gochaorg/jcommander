...
import Symbols.{Connector, Style, StyledSide}
import xyz.cofe.term.common.Position

import HVLine._

/** Горизонтальная/Вертикальная линиия определенной толщины */
case class HVLine( a:Position, b:Position, style:Style ) {
  ...
  def intersection( line: HVLine ):Option[Intersection] = {
    def pOrder : HVLine=>HVLine = l =>
      HVLine(
        Position( l.a.x min l.b.x, l.a.y min l.b.y ),
        Position( l.a.x max l.b.x, l.a.y max l.b.y ),
        l.style
      )
    ...
    if horiz && line.horiz then
      ...
      if hline.a.x <= vline.a.x && vline.a.x <= hline.b.x then
        if vline.a.y <= hline.a.y && hline.a.y <= vline.b.y then
          val p = Position(vline.a.x, hline.a.y)
